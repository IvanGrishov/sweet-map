  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-sm focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-100 transition-all relative">
      ...
      <Transition name="dropdown">
        <ul
          v-if="showDropdown"
          class="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden"
        >
          <li
    ...
    <Transition name="strip">
      <div
        ...
        class="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-xl px-3 py-2"
      >
        <svg class="w-3.5 h-3.5 text-indigo-500 shrink-0" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
        </svg>
        <span class="flex-1 text-xs text-indigo-700 truncate">{{ shortTitle(selectedResult.display_name) }}</span>
        <button
          class="text-xs font-semibold text-white bg-indigo-500 hover:bg-indigo-600 transition-colors px-2.5 py-1 rounded-lg shrink-0"
          @click="addMarker"
        >
          + {{ t('admin.add_marker') }}
        </button>
